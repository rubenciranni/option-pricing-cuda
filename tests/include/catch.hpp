...
#define INTERNAL_CATCH_METHOD_AS_TEST_CASE(QualifiedMethod, ...)                               \
  ...
  Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                                    \
      Catch::makeTestInvoker(&QualifiedMethod), CATCH_INTERNAL_LINEINFO, "&" #QualifiedMethod, \
      Catch::NameAndTags{__VA_ARGS__});                                                        \
  } /* NOLINT */                                                                               \
  CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION
...
#define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE2(TestName, TestFuncName, Name, Tags, Signature, \
  ...
  struct TestName {                                                                               \
    void reg_tests() {                                                                            \
      ...
      (void)expander{                                                                             \
          (Catch::AutoReg(                                                                        \
               Catch::makeTestInvoker(&TestFuncName<Types>), CATCH_INTERNAL_LINEINFO,             \
               Catch::StringRef(),                                                                \
...
#define INTERNAL_CATCH_TEMPLATE_LIST_TEST_CASE_2(TestName, TestFunc, Name, Tags, TmplList)       \
  ...
  struct TestName {                                                                              \
    void reg_tests() {                                                                           \
      ...
      (void)expander{                                                                            \
          (Catch::AutoReg(                                                                       \
               Catch::makeTestInvoker(&TestFunc<Types>), CATCH_INTERNAL_LINEINFO,                \
               Catch::StringRef(),                                                               \
...
struct IMutableEnumValuesRegistry {
  ...
  Detail::EnumInfo const& registerEnum(StringRef enumName, StringRef allEnums,
    ...
    for (auto enumValue : values) intValues.push_back(static_cast<int>(enumValue));
...
std::string rangeToString(InputIterator first, Sentinel last) {
  ...
  if (first != last) {
    ...
    for (++first; first != last; ++first) rss << ", " << ::Catch::Detail::stringify(*first);
  }
  rss << " }";
  return rss.str();
}
...
// use of const for Object pointers is very uncommon and under ARC it causes some kind of signature
// mismatch that breaks compilation
template <>
...
class RepeatGenerator : public IGenerator<T> {
 ...
 public:
  ...
  bool next() override {
    ...
    // If it happily moves, we are left in that state, otherwise it is time to start reading from
    // our cache
    if (m_current_repeat == 0) {
      const auto success = m_generator.next();
      if (!success) {
    ...
    // In the second case, we need to move indices forward and check that we haven't run up against
    // the end
    ++m_repeat_index;
...
GeneratorWrapper<T> map(Func&& function, GeneratorWrapper<U>&& generator) {
  return GeneratorWrapper<T>(pf::make_unique<MapGenerator<T, U, Func>>(std::forward<Func>(function),
                                                                       std::move(generator)));
}
...
struct CumulativeReporterBase : IStreamingReporter {
  ...
  void sectionStarting(SectionInfo const& sectionInfo) override {
    ...
    if (m_sectionStack.empty()) {
      if (!m_rootSection) m_rootSection = std::make_shared<SectionNode>(incompleteStats);
      node = m_rootSection;
    } else {
...
#define INTERNAL_CATCH_BENCHMARK(BenchmarkName, name, benchmarkIndex) \
  if (Catch::Benchmark::Benchmark BenchmarkName{name}) BenchmarkName = [&](int benchmarkIndex)
...
double erf_inv(double x) {
  // Code accompanying the article "Approximating the erfinv function" in GPU Computing Gems, Volume
  // 2
  double w, p;
...
std::unique_ptr<EnumInfo> makeEnumInfo(StringRef enumName, StringRef allValueNames,
  ...
  std::size_t i = 0;
  for (auto value : values) enumInfo->m_values.emplace_back(value, valueNames[i++]);
...
std::size_t listTestsNamesOnly(Config const& config) {
  ...
  for (auto const& testCaseInfo : matchedTestCases) {
    ...
    if (startsWith(testCaseInfo.name, '#'))
      ...
      Catch::cout() << testCaseInfo.name;
    if (config.verbosity() >= Verbosity::High) Catch::cout() << "\t@" << testCaseInfo.lineInfo;
    Catch::cout() << std::endl;
  }
  return matchedTests;
}
...
Option<std::size_t> list(std::shared_ptr<Config> const& config) {
  ...
  if (config->listTests()) listedCount = listedCount.valueOr(0) + listTests(*config);
  if (config->listTestNamesOnly())
    listedCount = listedCount.valueOr(0) + listTestsNamesOnly(*config);
  if (config->listTags()) listedCount = listedCount.valueOr(0) + listTags(*config);
  if (config->listReporters()) listedCount = listedCount.valueOr(0) + listReporters();
  return listedCount;
}
...
void write(std::ostream& out, FloatingPoint num) {
  out << std::scientific << std::setprecision(std::numeric_limits<FloatingPoint>::max_digits10 - 1)
      << num;
}
...
void RunContext::assertionEnded(AssertionResult const& result) {
  ...
  // We have no use for the return value (whether messages should be cleared), because messages were
  // made scoped and should be let to clear themselves out.
...
RegistrarForTagAliases::RegistrarForTagAliases(char const* alias, char const* tag,
  ...
  CATCH_CATCH_ALL {
    // Do not throw when constructing global objects, instead register the exception to be processed
    // later
    getMutableRegistryHub().registerStartupException();
  }
}
...
void SectionTracker::addNextFilters(std::vector<std::string> const& filters) {
  if (filters.size() > 1) m_filters.insert(m_filters.end(), filters.begin() + 1, filters.end());
}
...
TestSpec::Matches TestSpec::matchesByFilter(std::vector<TestCase> const& testCases,
  ...
  std::transform(m_filters.begin(), m_filters.end(), matches.begin(), [&](Filter const& filter) {
    ...
    for (auto const& test : testCases)
      if (isThrowSafe(test, config) && filter.matches(test)) currentMatches.emplace_back(&test);
...
void TestSpecParser::addNamePattern() {
  ...
  if (!token.empty()) {
    ...
    if (m_exclusion) pattern = std::make_shared<TestSpec::ExcludedPattern>(pattern);
  ...
  ConsoleAssertionPrinter(std::ostream& _stream, AssertionStats const& _stats,
    ...
    switch (result.getResultType()) {
      ...
      case ResultWas::ExplicitFailure:
        passOrFail = "FAILED";
        colour = Colour::Error;
        if (_stats.infoMessages.size() == 1) messageLabel = "explicitly with message";
        if (_stats.infoMessages.size() > 1) messageLabel = "explicitly with messages";
        break;
        // These cases are here to prevent compiler warnings
      case ResultWas::Unknown:
      case ResultWas::FailureBit:
...
bool ConsoleReporter::assertionEnded(AssertionStats const& _assertionStats) {
  ...
  if (!includeResults && result.getResultType() != ResultWas::Warning) return false;
...
void JunitReporter::writeAssertions(SectionNode const& sectionNode) {
  for (auto const& assertion : sectionNode.assertions) writeAssertion(assertion);
}
...
bool XmlReporter::assertionEnded(AssertionStats const& assertionStats) {
  ...
  // Drop out if result was successful but we're not printing them.
  if (!includeResults && result.getResultType() != ResultWas::Warning) return true;
