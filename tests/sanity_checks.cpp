...
std::vector<SingleRun> TESTS= {
    SingleRun(100, 100, 0.5, 0.03, 0.2, 0.015, 60, OptionType::Put)
};
...
bool run_sanity_checks_by_function(const PricingFunction fun) {
    ...
    for (const auto&  run : TESTS) {
        TESTS_WITH_RESULTS.push_back({ binomial_crr_american_vanilla_option_cpu(
            run.S, run.K, run.T, run.r, run.sigma, run.q, run.n, run.type
        ), run});
    }
