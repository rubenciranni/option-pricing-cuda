...
int main(int argc, char** argv) {
    ...
    // Pricing subcommand
    std::string option_type_str, exercise_type_str, pricing_method_str, backend_str, output_format_str;
    ...
    benchmark_subcommand
        ...
        ->default_val(false);
    benchmark_subcommand->add_option("--output-format", output_format_str, "Output Format (pprint|json)")
        ->default_val("pprint")
    ...
    if (*price_subcommand) {
        ...
        if (output_format == OutputFormat::JSON) {
            ...
            for (const auto& res : results) {
                ...
                std::string json_hyperparam_delimiter = "_@", function_id=res.function_name, hyperparam_id="null", param_1="null", param_2="null", param_3="null", param_4="null", param_5="null";
                if (res.function_name.find(json_hyperparam_delimiter) != std::string::npos) {
                    int json_delimiter_start = 0, json_delimiter_end = res.function_name.find(json_hyperparam_delimiter);
                    function_id = res.function_name.substr(json_delimiter_start, json_delimiter_end-json_delimiter_start);
                    ...
                    hyperparam_id = res.function_name.substr(json_delimiter_start, json_delimiter_end-json_delimiter_start);
                    json_delimiter_start = json_delimiter_end + 1;
                    
                    json_delimiter_end = res.function_name.find('_', json_delimiter_start);
                    param_1 = res.function_name.substr(json_delimiter_start, json_delimiter_end-json_delimiter_start);
                    json_delimiter_start = json_delimiter_end + 1;
                    
                    json_delimiter_end = res.function_name.find('_', json_delimiter_start);
                    param_2 = res.function_name.substr(json_delimiter_start, json_delimiter_end-json_delimiter_start);
                    json_delimiter_start = json_delimiter_end + 1;
                    
                    json_delimiter_end = res.function_name.find('_', json_delimiter_start);
                    param_3 = res.function_name.substr(json_delimiter_start, json_delimiter_end-json_delimiter_start);
                    json_delimiter_start = json_delimiter_end + 1;
                    
                    json_delimiter_end = res.function_name.find('_', json_delimiter_start);
                    param_4 = res.function_name.substr(json_delimiter_start, json_delimiter_end-json_delimiter_start);
                    json_delimiter_start = json_delimiter_end + 1;
                    
                    param_5 = res.function_name.substr(json_delimiter_start, res.function_name.length() - json_delimiter_start);
                }
                json_output += std::string(json_depth, '\t') + "\"id\": \"" + res.function_name + "\",\n";
                json_output += std::string(json_depth, '\t') + "\"function_id\": \"" + function_id + "\",\n";
                json_output += std::string(json_depth, '\t') + "\"do_pass_sanity_check\": \"" + (res.pass_sanity_check ? "true" : "false") + "\",\n";
                json_output += std::string(json_depth, '\t') + "\"hyperparam_id\": \"" + hyperparam_id + "\",\n";
                json_output += std::string(json_depth, '\t') + "\"hyperparam_1\": \"" + param_1 + "\",\n";
                json_output += std::string(json_depth, '\t') + "\"hyperparam_2\": \"" + param_2 + "\",\n";
                json_output += std::string(json_depth, '\t') + "\"hyperparam_3\": \"" + param_3 + "\",\n";
                json_output += std::string(json_depth, '\t') + "\"hyperparam_4\": \"" + param_4 + "\",\n";
                json_output += std::string(json_depth, '\t') + "\"hyperparam_5\": \"" + param_5 + "\",\n";
                ...
                json_output += std::string(json_depth, '\t') + "\"n\": [" + json_nsteps_string.substr(2, json_nsteps_string.length()-2) + "],\n";
                json_output += std::string(json_depth, '\t') + "\"time_ms\": [" + json_time_string.substr(2, json_time_string.length()-2) + "],\n";
                json_output += std::string(json_depth, '\t') + "\"price\": [" + json_price_string.substr(2, json_price_string.length()-2) + "]\n";
