...
// Kernel with precomputed payoff values (no first layer kernel needed - computed on CPU)
...
double vanilla_american_binomial_cuda_precomputed_stock_price(const double S, const double K,
  ...
  first_layer_kernel_precomputed_stock_price<<<num_blocks, num_threads>>>(d_option_values, n,
...
double vanilla_american_binomial_cuda_precomputed_payoffs(const double S, const double K,
  ...
  // Compute positive powers and payoffs simultaneously: S*u^1, S*u^2, ..., S*u^n
  double S_up = S;
  for (int i = 1; i <= n; i++) {
    S_up *= u;
  ...
  // Compute negative powers and payoffs simultaneously: S*u^(-1), S*u^(-2), ..., S*u^(-n)
  double S_down = S;
  for (int i = 1; i <= n; i++) {
    S_down *= d;
