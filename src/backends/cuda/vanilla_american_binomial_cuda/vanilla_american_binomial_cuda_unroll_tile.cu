...
#pragma unroll
    for (int delta_id = 0; delta_id <= delta_level; delta_id++) {
      ...
      res[delta_id] = fmax(st_buffer[INDEX_BUFFER(exponent, THREADS_PER_BLOCK + UNROLL_FACTOR)],
...
#pragma unroll
  for (int i = threadIdx.x; i <= 2 * (UNROLL_FACTOR + THREADS_PER_BLOCK); i += THREADS_PER_BLOCK) {
