...
std::vector<double> FUNC_NAME(vanilla_american_binomial_cuda_batch)(
    ...
    int num_blocks;
    std::vector<double*> layer_values_read_ds, layer_values_write_ds, st_buffer_bank0_ds,
        st_buffer_bank1_ds;
    for (size_t i = 0; i < runs.size(); i++) {
        auto run = runs[i];
        ...
        FUNC_NAME(fill_st_buffer_kernel)<<<num_blocks, THREADS_PER_BLOCK, 1, stream>>>(
            st_buffer_bank0_d, run.S, run.K, u, sign, run.n, 0);
        FUNC_NAME(fill_st_buffer_kernel)<<<num_blocks, THREADS_PER_BLOCK, 1, stream>>>(
            st_buffer_bank1_d, run.S, run.K, u, sign, run.n, 1);
    ...
    for (size_t i = 0; i < runs.size(); i++) {
        ...
        for (; level >= UNROLL_FACTOR && level > 1; level -= UNROLL_FACTOR) {
            num_blocks = std::ceil((level - UNROLL_FACTOR + 1) * 1.0 / OUTPUTS_PER_BLOCK);
            FUNC_NAME(compute_next_layers_kernel)<<<num_blocks, THREADS_PER_BLOCK>>>(
                layer_values_read_ds[i], layer_values_write_ds[i], st_buffer_bank0_ds[i],
                st_buffer_bank1_ds[i], up, down, level - UNROLL_FACTOR, run.n);
        ...
        for (; level >= 1; level -= 1) {
            ...
            FUNC_NAME(compute_next_layer_kernel)<<<num_blocks, THREADS_PER_BLOCK>>>(
                layer_values_read_ds[i], layer_values_write_ds[i], st_buffer_bank0_ds[i],
                st_buffer_bank1_ds[i], up, down, level - 1, run.n);
            std::swap(layer_values_read_ds[i], layer_values_write_ds[i]);
        }
    }
    ...
    for (size_t i = 0; i < runs.size(); i++) {
        ...
        auto stream = streams[i];
        double value_h;
        cudaMemcpyAsync(&value_h, layer_values_read_ds[i], (1) * sizeof(double),
